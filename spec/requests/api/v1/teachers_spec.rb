...
RSpec.describe "Api::V1::Teachers", type: :request do
  describe "GET /index" do
    ...
    let!(:teaching_assignment1) do
      create(:teaching_assignment, user: teacher1, student: student1)
    end
    let!(:teaching_assignment2) do
      create(:teaching_assignment, user: teacher2, student: student2)
    end
    let!(:teaching_assignment_admin) do
      create(:teaching_assignment, user: admin_user, student: student1)
    end
    ...
    context "signed in user" do
      ...
      it "returns all teachers for the school" do
        ...
        expect(
          json_response["current_user"]["students"].map { |s| s["id"] }
        ).to include(student1.id)

        expect(json_response["teachers"].size).to eq(2)
        expect(json_response["teachers"].map { |t| t["id"] }).to include(
          teacher1.id,
          teacher2.id
        )
        # map を2回使うと、[[student1.id], [student2.id]] のような配列ができてincludeがうまくいかないので、flat_mapを使う
        expect(
          json_response["teachers"].flat_map do |t|
            t["students"].map { |s| s["id"] }
          end
        ).to include(student1.id, student2.id)
      end
